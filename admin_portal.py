...
def tools_page(request: Request):
    ...
    with db() as conn:
        ...
        cards=[]
        for t in tools:
            current=latest_release(conn,t['id']); version=current['version'] if current else '未发布'; developer=t['developer_name'] or '未绑定'; price=int(t['price_cents'] or 0)/100
            status='<span class="badge current">已上架</span>' if t['active'] else '<span class="badge off">已下架</span>'
            cards.append(f'''<div class="admin-tool"><h2>{esc(t['name'])}</h2><div class="meta">{esc(t['category'])} · {esc(t['platform'])} · {status}</div><div class="stats"><div class="stat"><b>v{esc(version)}</b><span>版本</span></div><div class="stat"><b>¥{price:.2f}</b><span>售价</span></div><div class="stat"><b>{esc(developer)}</b><span>开发者</span></div><div class="stat"><b>{int(t['downloads']):,}</b><span>下载</span></div></div><div class="admin-actions"><a class="btn" href="/manage/tools/{esc(t['slug'])}">管理</a><a class="btn secondary" href="/tools/{esc(t['slug'])}" target="_blank">前台</a></div></div>''')
...
def tool_admin(request: Request, slug: str):
    ...
    version=current['version'] if current else '未发布'; developer=t['developer_name'] or '未绑定开发者'; price=int(t['price_cents'] or 0)/100
    body=f'''<div class="admin-shell"><a class="backlink" href="/manage/tools/">← 返回工具管理</a><div class="admin-head"><div><h1>{esc(t['name'])}</h1><div class="admin-sub">v{esc(version)} · {int(t['downloads']):,} 次下载</div></div><a class="btn secondary" href="/tools/{esc(t['slug'])}" target="_blank">查看前台</a></div><div class="split"><div class="admin-card"><h2>产品资料</h2><form action="/manage/tools/{esc(t['slug'])}/update" method="post"><div class="field"><label>工具名称</label><input name="name" value="{esc(t['name'])}" required></div><div class="field"><label>一句话介绍</label><input name="tagline" value="{esc(t['tagline'])}"></div><div class="field"><label>分类</label><input name="category" value="{esc(t['category'])}"></div><div class="field"><label>平台</label><input name="platform" value="{esc(t['platform'])}"></div><div class="field"><label>图标文字</label><input name="icon_text" value="{esc(t['icon_text'])}"></div><div class="field"><label>小游侠价格（元）</label><input name="price_yuan" type="number" min="0" step="0.01" value="{price:.2f}"></div><div class="field"><label>上架状态</label><select name="active"><option value="1" {'selected' if t['active'] else ''}>上架</option><option value="0" {'selected' if not t['active'] else ''}>下架</option></select></div><div class="field"><label>完整介绍</label><textarea name="description">{esc(t['description'])}</textarea></div><div class="field"><label>截图 URL</label><textarea name="screenshots">{esc(t['screenshots'])}</textarea></div><button class="btn">保存</button></form></div><div class="admin-card"><h2>归属</h2><div class="owner-row"><b>{esc(developer)}</b><span>原开发者</span></div><div class="owner-row"><b>v{esc(version)}</b><span>当前版本</span></div><div class="owner-row"><b>¥{price:.2f}</b><span>售价</span></div><p class="muted">新版本由原开发者在个人中心提交，管理员在投稿审核中通过。</p></div></div><div style="height:16px"></div><div class="admin-card"><h2>版本历史</h2><table class="table"><thead><tr><th>版本</th><th>状态</th><th>日期</th><th>大小</th><th>说明</th></tr></thead><tbody>{rows}</tbody></table></div></div>'''
...
def update_tool(request: Request, slug: str, name: str=Form(...), tagline: str=Form(''), description: str=Form(''), category: str=Form('效率工具'), platform: str=Form('Windows'), icon_text: str=Form('100'), screenshots: str=Form(''), price_yuan: float=Form(0), active: int=Form(1)):
    require_admin(request)
    if price_yuan<0 or active not in (0,1): raise HTTPException(400,"参数不正确。")
    with db() as conn:
        t=conn.execute("SELECT id FROM tools WHERE slug=?",(slug,)).fetchone()
        if not t: raise HTTPException(404,"工具不存在。")
        conn.execute("UPDATE tools SET name=?,tagline=?,description=?,category=?,platform=?,icon_text=?,screenshots=?,price_cents=?,active=?,updated_at=? WHERE id=?",(name.strip(),tagline.strip(),description.strip(),category.strip(),platform.strip(),icon_text.strip(),screenshots.strip(),int(round(price_yuan*100)),active,now_text(),t['id']))
