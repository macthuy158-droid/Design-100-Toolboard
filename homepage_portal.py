...
.home-bounties{padding:16px 0 20px}
    ...
    def homepage():
        ...
        with site.db() as conn:
            ...
            for i, t in enumerate(tools, 1):
                ...
                search_text = f"{t['name']} {t['tagline']} {category} {platform_label}".lower()
                ...
                cards.append(
                    f'''<a class="tool" href="/tools/{site.esc(t['slug'])}" data-category="{site.esc(category)}" data-search="{site.esc(search_text)}">
...
<div class="chips"><span class="chip">{site.esc(category)}</span><span class="chip">{site.esc(platform_label)}</span><span class="chip">v{site.esc(version)}</span></div>
<div class="footrow">{foot}<div class="arrow">↗</div></div></a>'''
        ...
        filters += [
            f'<button class="filter" data-category="{site.esc(c)}">{site.esc(c)}</button>'
            for c in categories
        ]
...
<label class="searchbox home-search" aria-label="搜索工具"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-4.2-4.2"></path></svg><input id="tool-search" placeholder="搜索工具、分类或关键词"></label></div></div>
...
<section id="tools"><div class="toolbar home-tools-toolbar"><div><h2>Tool Board</h2><p>所有已发布工具 · 按累计下载量排序</p></div><div class="filters">{''.join(filters)}</div></div>
...
<footer id="about" class="footer"><span><strong>深圳院设计100%</strong> · 工具开发板</span><span>面向真实设计生产流程持续迭代 · Internal Design Tools</span></footer></div>'''
        return HTMLResponse(
            site.page(body, extra=HOME_CSS + site.JS),
