...
# 小游侠 (院外) signup is closed. Flip TOOLBOARD_GUEST_SIGNUP=1 in
# /etc/design100-toolboard.env to reopen it without a code change.
GUEST_SIGNUP_OPEN = os.getenv("TOOLBOARD_GUEST_SIGNUP", "0") == "1"
...
def login_page(request: Request, next: str = "/"):
    ...
    <div class="field"><label>密码</label><input name="password" type="password" required></div><button class="btn" type="submit">登录</button> <a class="btn secondary" href="/account/register">注册</a></form></div></div>'''
...
def register_page():
    guest_picker = guest_form = ""
    if GUEST_SIGNUP_OPEN:
        guest_picker = ("""<div class="field"><label>选择身份</label><select id="role-select" onchange="document.getElementById('form-xiaofeixia').style.display=this.value==='xiaofeixia'?'block':'none';document.getElementById('form-xiaoyouxia').style.display=this.value==='xiaoyouxia'?'block':'none'">"""
                        """<option value="xiaofeixia">小飞侠（院内开发者）</option><option value="xiaoyouxia">小游侠（同行用户）</option></select></div>""")
        guest_form = ("""<form id="form-xiaoyouxia" action="/account/register" method="post" style="display:none"><input type="hidden" name="role" value="xiaoyouxia">"""
                      """<div class="field"><label>昵称</label><input name="display_name" required></div>"""
                      ...
                      """<button class="btn">注册小游侠</button></form>""")
    body=f'''<div class="loginwrap"><div class="login"><div class="eyebrow">DESGIN 100%</div><h1>注册</h1>
    <div class="notice">小飞侠：院内开发者，凭邀请码注册。注册即赠送 {coin_core.REGISTER_BONUS} 个飞侠币，可用于下载他人工具或发布悬赏需求。</div>
    {guest_picker}{guest_form}
...
def register(request: Request, role: str = Form("xiaofeixia"), display_name: str = Form(""), email: str = Form(""), password: str = Form(...), real_name: str = Form(""), invite_code: str = Form("")):
    core.init_db()
    # Hiding the form is not enough — a hand-rolled POST would sail past it.
    if role != core.ROLE_XIAOFEIXIA and not GUEST_SIGNUP_OPEN:
        raise HTTPException(403,"小游侠注册通道暂未开放。")
    ...
    if role == core.ROLE_XIAOFEIXIA:
        ...
        try:
            with site.db() as conn:
                ...
                coin_core.grant_register_bonus(conn, uid)
        except Exception:
...
def me(request: Request):
    ...
    with site.db() as conn:
        ...
        coin_name=coin_core.COIN_NAME
        coin_balance=coin_core.balance(conn,user['id'])
        coin_rows=coin_core.recent(conn,user['id'])
        coin_earned=coin_core.earned(conn,user['id'])
    ...
    for t in owned:
        ...
        <div class="dev-stats"><div class="dev-stat"><b>v{site.esc(version)}</b><span>当前版本</span></div><div class="dev-stat"><b>{coin_core.coin_price(t)}</b><span>下载飞侠币</span></div><div class="dev-stat"><b>{int(t['downloads'] or 0):,}</b><span>累计下载</span></div><div class="dev-stat"><b>{rating} / 5</b><span>{reviews} 条评价</span></div><div class="dev-stat"><b>{paid_orders}</b><span>已确认购买</span></div><div class="dev-stat"><b>¥{paid_amount:.2f}</b><span>已确认销售额</span></div></div>
        <form class="coin-price-form" action="/account/tools/{site.esc(t['slug'])}/coin-price" method="post"><label>下载价格</label><input name="feixia_coin_price" type="number" min="0" max="{coin_core.MAX_PRICE}" step="1" value="{coin_core.coin_price(t)}" title="他人下载需支付的飞侠币，0 为免费"><span class="coin-unit">飞侠币（0 = 免费）</span><button class="btn secondary">保存</button></form>
    ...
    wallet_intro=(f'注册赠送 {coin_core.REGISTER_BONUS} 个；工具审核通过再得 {coin_core.PUBLISH_REWARD} 个。'
                  f'你的工具被别人下载时，对方支付的{coin_core.COIN_NAME}归你；下载他人工具或发布悬赏则消耗。')
...
def coin_unlock(request: Request, slug: str):
    """Spend 飞侠币 to unlock another developer's tool."""
    user = login_required(request)
    with site.db() as conn:
        tool = conn.execute("SELECT * FROM tools WHERE slug=? AND active=1", (slug,)).fetchone()
        if not tool:
...
def set_coin_price(request: Request, slug: str, feixia_coin_price: int = Form(0)):
    """Tool owners set what other developers pay to download."""
    ...
    with site.db() as conn:
        ...
        conn.execute("UPDATE tools SET feixia_coin_price=?,updated_at=? WHERE id=?",
                     (feixia_coin_price, core.now(), tool['id']))
...
def install_public_routes(app):
    ...
    def detail(request: Request, slug: str):
        ...
        with site.db() as conn:
            ...
            coin_name=coin_core.COIN_NAME
            coin_balance=coin_core.balance(conn,user['id']) if user else 0
        ...
        if is_web:
            ...
            hint='' if affordable else f'　发布工具可获得{coin_name}。'
        ...
        if is_web:
            ...
            obtain_text=f'支付 {coin_cost} {coin_name}解锁下载，解锁一次后永久可用。\n你支付的{coin_name}归该工具的开发者所有。'
            obtain_price=f'{coin_cost} {coin_name}'
        else:
            obtain_text=f'该工具免费，登录后即可下载。'
            obtain_price='免费'
